...
def dijkstra_shortest_path(graph, start):
    """
    Compute the shortest path from a start node to all other nodes in a weighted graph using Dijkstra's algorithm.
    
    :param graph: A dictionary representing the graph where each key is a node and its value is a list of tuples 
                  (neighbor, weight) indicating adjacent nodes and edge weights.
    :param start: The starting node from which to compute the shortest paths.
    :return: A dictionary mapping each node to its shortest distance from the start node. Unreachable nodes have a distance of float('inf').
    """
